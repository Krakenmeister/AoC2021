...
int main() {
	...
	while (getline(input, line)) {
		...
		} else if (abs(coordinates[0][0] - coordinates[1][0]) == abs(coordinates[0][1] - coordinates[1][1])) {
			const int startX = min(coordinates[0][0], coordinates[1][0]);
			const int endX = max(coordinates[0][0], coordinates[1][0]);
			const int startY = (coordinates[0][0] < coordinates[1][0]) ? coordinates[0][1] : coordinates[1][1];
			const int directionY = (int) ((coordinates[0][1] - coordinates[1][1]) / (coordinates[0][0] - coordinates[1][0]));
			for (int i = startX; i <= endX; i++) {
				const pair<int, int> currentPoint = {i, startY + ((i - startX) * directionY)};
				if (pointCount.find(currentPoint) == pointCount.end()) {
					pointCount[currentPoint] = 1;
				} else {
					pointCount[currentPoint]++;
